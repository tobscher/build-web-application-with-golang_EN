...
##Produce JSON
In many situations, we need to produce JSON data and responde to clients. In Go, JSON package has a function called `Marshal` to do this job:
...
- Field tag contains `"-"` will not be outputed.
- If tag contains customized name, Go uses this instead of field name, like `serverName` in above example.
- If tag contains `omitempty`, this field will not be outpued if it is its zero-value.
...
- Do not try to encode nested data, it leds dead loop when produce JSON data.
