#7.4 Templates
##What is tempale?
I believe you've heard MVC design model, where Model processes data, View shows results, Controller handles user requests. As for View level. Many dynamic languages generate data by writing code in static HTML files, like JSP implements by insearting `<%=....=%>`, PHP implements by insearting `<?php.....?>`.
...
Most of content that web applications responde to clients is static, and dynamic part is usually small. For example, you need to show a list of visited users, only user name is dynamic, and style of list is always the same, so template is for reusing static content. 
